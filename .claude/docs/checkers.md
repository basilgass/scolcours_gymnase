...
| ExactChecker      | `resources/js/Checkers/Basic/ExactChecker.ts`      | 2026-08-01                 |
...
| AngleChecker      | `resources/js/Checkers/Basic/AngleChecker.ts`      | —                          |
...
> Mis à jour le 2026-04-07. Config par défaut : ExactChecker, dimension 1 acceptée. Config `co` : colinéarité. Configs
`nb`/`frac` héritées de CoordChecker.
...
> Mis à jour le 2026-04-07. Format attendu : `a*10^b` avec `1 ≤ |a| < 10`. Config `[n]` : n chiffres significatifs (
> délégué au NumberChecker). Config `s` : strict.
...
### ExactChecker — Tableau des contrôles

> Mis à jour le 2026-08-01. Compare deux valeurs exactes après normalisation (`normalizeExpression`) puis
> évaluation numérique via `NumExp` (fonctions supportées : `sin`/`cos`/`tan` en radians, `sqrt`, `nthrt`,
> `ln` népérien, `log` base 10, `logn(value,base)` base n ; constantes `pi`, `e` ; puissances rationnelles). Config par défaut : forme
> exacte attendue. Config `soft` : toute écriture de même valeur est acceptée. Le préfixe `#` de la réponse
> donnée est retiré avant contrôle.

| Bonne réponse            | Mauvaise réponse                    | Score | Message renvoyé                                                              |
|--------------------------|-------------------------------------|-------|------------------------------------------------------------------------------|
| `2`                      | `3xsi`                              | 0     | "La réponse n'est pas une valeur exacte reconnue." *(entrée invalide)*       |
| `2`                      | `(((`                               | 0     | "La réponse n'est pas une valeur exacte reconnue." *(parsing échoué, sans crash)* |
| `(((` *(attendu malformé)* | `2`                               | 0     | "La réponse attendue n'est pas une valeur exacte reconnue." *(erreur de config)* |
| `3`                      | `logn(8,1)`                         | 0     | "La réponse donnée n'est pas juste." *(base invalide n=1 → NaN)*             |
| `1/2`                    | `3/4`                               | 0     | "La réponse donnée n'est pas juste." *(valeur numérique différente)*         |
| `2`                      | `8^(1/3)` (défaut)                  | 0.3   | "La réponse donnée est juste, mais pas sous la forme attendue."              |
| `1/2`                    | `2/4` (défaut)                      | 0.3   | "...pas sous la forme attendue.<br/>Il faut réduire la fraction."            |
| `sqrt(2)/2`              | `1/sqrt(2)` (défaut)                | 0.3   | "...pas sous la forme attendue.<br/>Il y a encore une racine au dénominateur" |
| `3`                      | `logn(8,2)` (défaut)                | 0.3   | "La réponse donnée est juste, mais pas sous la forme attendue."              |
| `1/2`                    | `1/2`                               | 1     | *(succès — identique)*                                                       |
| `2sqrt(2)`               | `2sqrt2`                            | 1     | *(succès — même chaîne après normalisation)*                                |
| `1/2`                    | `#1/2`                              | 1     | *(succès — préfixe `#` retiré)*                                              |
| `2`                      | `8^(1/3)` (config `soft`)           | 1     | *(succès — même valeur, `9^(1/2)`=`3`, `2^(-1)`=`1/2`, `root(3)(8)`=`2`)*    |
| `2`                      | `log(100)` (config `soft`)          | 1     | *(succès — `ln(e)`=`1`, `sin(pi/6)`=`1/2`, `cos(0)`=`1`)*                    |
| `(1+sqrt(2))/2`          | `(3+sqrt(18))/6` (config `soft`)    | 1     | *(succès — écriture rationalisée équivalente)*                              |
| `3`                      | `logn(8,2)` (config `soft`)         | 1     | *(succès — log base n ; `logn(25,5)`=`2`, `logn(8,1/2)`=`-3`)*              |
