...
export const IllustrationsModules = import.meta.glob(['./Components/Posts/Illustrations/Elements/*.vue'])
...
export function getModule(key: string, type: MODULE_TYPES) {
    if(dynamicModules.hasOwnProperty(type) &&
    dynamicModules[type].modules.hasOwnProperty(`${dynamicModules[type].path}${key}.vue`)
    ){
        return defineAsyncComponent(dynamicModules[type].modules[`${dynamicModules[type].path}${key}.vue`])
    }
}
