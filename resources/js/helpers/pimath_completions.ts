...
const signatureMap: Record<string, string> = {
	...
	"PiMath.Random.polynom": "polynom(config?: { degree?, min?, max?, letters?, negative?, zero?, allowNullMonom? })",
...
const completionMap: Record<string, Completion[]> = {
	...
	"PiMath.Numeric": [
		...
		{
			label: "pythagoreanTripletsWithTarget",
			type: "function",
			detail: "(target: number, targetIsSquare?: boolean) → [number, number, number][]"
		},
	],
	...
	"PiMathExt": [
		{
			label: "wrap",
			type: "function",
			detail: "(v: string | number) → string — entoure les négatifs de \\left( \\right)"
		},
