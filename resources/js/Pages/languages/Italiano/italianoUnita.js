// Units
const unit0 = [
	["undici", "onze"],
	["la pagina", "la page"],
	["ventisette", "vingt-sept"],
	["la classe", "la classe"],
	["portoghese, i", "portugais"],
	["nove", "neuf"],
	["americano, a, i, e", "américain"],
	["il nome", "le nom"],
	["come", "comment"],
	["dire (v. irregolare)", "dire"],
	["l'attività", "l'activité"],
	["la risposta", "la réponse"],
	["marocchino, a, i, e", "marocain"],
	["due", "deux"],
	["sbagliato, a, i, e", "FAUX"],
	["chi", "qui"],
	["chiedere (chiesto)", "demander"],
	["la compagna", "la camarade"],
	["la frase", "la phrase"],
	["Questo è Giovanni.", "Lui c'est Jean."],
	["formare", "former"],
	["Non mangio gli spaghetti.", "Je ne mange pas les spaghetti."],
	["il compito", "le devoir"],
	["uno", "un"],
	["il computer", "l'ordinateur"],
	["l'insegnante", "l'enseignante"],
	["dodici", "douze"],
	["la macchina", "la voiture"],
	["e", "et"],
	["la studentessa", "l'étudiante"],
	["Come ti chiami?", "Comment t'appelles-tu ?"],
	["diciassette", "dix-sept"],
	["questo, a, i, e", "ce, cette, ces"],
	["la tabella (femm.)", "le tableau"],
	["ripetere", "répéter"],
	["presentare", "présenter"],
	["prego", "de rien"],
	["straniero, a, i, e", "étranger"],
	["la persona", "la personne"],
	["la parola", "le, mot, la parole"],
	["completare", "compléter"],
	["l'immagine", "l'image"],
	["la sorella", "la soeur"],
	["tedesco, a, i, e", "allemand"],
	["l'età (femm.)", "l'âge"],
	["l'aggettivo", "l'adjectif"],
	["ciao", "salut"],
	["la sedia", "la chaise"],
	["quindici", "quinze"],
	["la professoressa", "la professeure"],
	["femminile, i", "féminin"],
	["avere (v. irregolare)", "avoir"],
	["(che) cosa", "que, quoi, qu'est-ce que"],
	["Chi sei?", "Qui es-tu ?"],
	["plurale, i", "pluriel"],
	["tredici", "treize"],
	["dieci", "dix"],
	["leggere (letto)", "lire"],
	["Ho 17 anni.", "J'ai 17 ans"],
	["o", "ou (bien)"],
	["lo studente", "l'étudiant"],
	["Le persone", "Les personnes"],
	["no", "non"],
	["la pronuncia", "la prononciation"],
	["spagnolo, a, i, e", "espagnol"],
	["quattordici", "quatorze"],
	["l'allieva", "l'élève (fém.)"],
	["quattro", "quatre"],
	["sei", "six"],
	["la canzone", "la chanson"],
	["il libro", "le livre"],
	["gli spaghetti", "les spaghetti"],
	["lo zaino", "le sac à dos"],
	["quanto, a, i, e", "combien (de)"],
	["piacere", "enchanté (e)"],
	["l'autobus", "le bus"],
	["svizzero, a, i, e", "suisse"],
	["zero", "zéro"],
	["grazie", "merci"],
	["salutare", "saluer"],
	["il banco", "le banc"],
	["il calcio", "le foot"],
	["la moda", "la mode"],
	["Quanti anni hai?", "Quel âge as-tu?"],
	["argentino, a, i, e", "argentin"],
	["fare lo spelling (v. irregolare)", "épeler"],
	["fare (v. irregolare)", "faire"],
	["otto", "huit"],
	["ascoltare", "écouter"],
	["ventinove", "vingt-neuf"],
	["ventotto", "vingt-huit"],
	["L'alfabeto", "L'alphabet"],
	["il numero", "le nombre"],
	["il fratello", "le frère"],
	["la penna", "la plume"],
	["La lingua e la scuola", "La langue et l'école"],
	["lavorare", "travailler"],
	["sì", "oui"],
	["il dialogo, i dialoghi", "le dialogue"],
	["il materiale", "le matériel"],
	["presentarsi*", "se présenter"],
	["sedici", "seize"],
	["singolare, i", "singulier"],
	["venticinque", "vingt-cinq"],
	["trenta", "trente"],
	["il sostantivo", "le substantif"],
	["Qual è", "quel(le) est"],
	["Benvenuti!", "Bienvenus!"],
	["Scusa!", "Excuse-moi !"],
	["il quaderno", "le cahier"],
	["fare una domanda (v. irregolare)", "poser une question"],
	["ventitré", "vingt-trois"],
	["la coppia(femm.)", "le couple"],
	["inglese, i", "anglais"],
	["l'esempio", "l'exemple"],
	["chiamarsi*", "s'appeler"],
	["finire (-isco)", "finir"],
	["chiamare", "appeler"],
	["Ti presento Carlo.", "Je te présente Carlo."],
	["ventuno", "vingt-et-un"],
	["la foto", "la photo"],
	["l'origine", "l'origine"],
	["l'allievo", "l'élève (masc.)"],
	["la lavagna (femm.)", "le tableau noir"],
	["scrivere (scritto)", "écrire"],
	["il gelato (masc.)", "la glace (miam!)"],
	["giusto, a, i, e", "juste"],
	["ventisei", "vingt-six"],
	["diciannove", "dix-neuf"],
	["la ragazza", "la fille"],
	["brasiliano, a, i, e", "brésilien"],
	["osservare", "observer"],
	["I numeri (1)", "Les nombres (1)"],
	["italiano, a, i, e", "italien"],
	["La nazionalità", "La nationalité"],
	["la consonante", "la consonne"],
	["il corso", "le cours"],
	["Come va?", "Comment ça va ?"],
	["il manuale", "le manuel"],
	["Scusi!", "Excusez-moi ! (politesse)"],
	["la domanda", "la question"],
	["essere* (v. irregolare)", "être"],
	["la scuola", "l'école"],
	["la lingua", "la langue"],
	["(Che) cosa mangi?", "Que manges-tu?"],
	["australiano, a, i, e", "australien"],
	["ventidue", "vingt-deux"],
	["il compagno", "le camarade"],
	["il suono", "le son"],
	["la vocale", "la voyelle"],
	["costruire (-isco)", "construire"],
	["ventiquattro", "vingt-quatre"],
	["il professore", "le professeur"],
	["il ragazzo", "le garçon"],
	["indicare", "indiquer"],
	["l'azione", "l'action"],
	["l'insegnante", "l'enseignant"],
	["l'aula", "la salle de classe"],
	["la cosa", "la chose"],
	["cominciare", "commencer"],
	["di niente", "de rien"],
	["non", "ne... pas"],
	["la lezione", "la leçon"],
	["tre", "trois"],
	["maschile, i", "masculin"],
	["lo sport", "le sport"],
	["il foglio (masc.)", "la feuille (papier)"],
	["Qual è il tuo numero di telefono?", "Quel est ton numéro de téléphone ?"],
	["la regola", "la règle"],
	["il panino", "le sandwich"],
	["diciotto", "dix-huit"],
	["per favore", "s'il te/vous plaît"],
	["sette", "sept"],
	["cinque", "cinq"],
	["la pizza", "la pizza"],
	["venti", "vingt"],
	["la lettera", "la lettre"],

]
const unit1 = [
	["Un nuovo inizio", "Un nouveau départ"],
	["Il corpo", "Le corps"],
	["l'uomo, gli uomini", "l'homme"],
	["la donna", "la femme"],
	["il fisico", "le physique"],
	["il viso", "le visage"],
	["la faccia, -ce", "la face, le visage"],
	["il capello", "le cheveu"],
	["l'occhio", "l'œil"],
	["la bocca, -che", "la bouche"],
	["il naso", "le nez"],
	["la testa", "la tête"],
	["la fronte (femm.)", "le front"],
	["il braccio, le braccia", "le bras"],
	["il dito, le dita", "le doigt"],
	["il ginocchio, le ginocchia", "le genou"],
	["l'orecchio, le orecchie", "l'oreille"],
	["la gamba", "la jambe"],
	["il piede", "le pied"],
	["la pelle", "la peau"],
	["i baffi", "les moustaches"],
	["la barba", "la barbe"],
	["gli occhiali", "les lunettes"],
	["Il lavoro", "Le travail"],
	["l'inizio", "le début"],
	["la fine", "la fin"],
	["la notizia", "la nouvelle"],
	["il direttore", "le directeur"],
	["la direttrice", "la directrice"],
	["l'agenzia", "l'agence"],
	["il collega", "le collègue"],
	["la collega", "la collègue"],
	["l'ufficio", "le bureau"],
	["l'informazione", "l'information"],
	["il giornale", "le journal"],
	["l'appuntamento", "le rendez-vous"],
	["l'amore", "l'amour"],
	["il ristorante", "le restaurant"],
	["la città", "la ville"],
	["l'università", "l'université"],
	["il bar", "le bar"],
	["la biblioteca", "la bibliothèque"],
	["la regione", "la région"],
	["Le caratteristiche fisiche", "Les caractéristiques physiques"],
	["giovane, i", "jeune"],
	["vecchio, a, i, e", "vieux"],
	["bello, a, i, e", "beau"],
	["brutto, a, i, e", "laid"],
	["carino, a, i, e", "joli, mignon"],
	["riccio, a, i, e", "frisé"],
	["liscio, a, i, e", "lisse"],
	["corto, a, i, e", "court"],
	["lungo, a, i, e", "long"],
	["piccolo, a, i, e", "petit"],
	["grande, i", "grand"],
	["alto, a, i, e", "haut, de grande taille"],
	["basso, a, i, e", "bas, de petite taille"],
	["magro, a, i, e", "maigre"],
	["grasso, a, i, e", "gras"],
	["calvo, a, i, e", "chauve"],
	["scuro, a, i, e", "foncé"],
	["chiaro, a, i, e", "clair"],
	["Il carattere", "Le caractère"],
	["gentile, i", "gentil"],
	["cattivo, a, i, e", "méchant"],
	["simpatico, a, i, e", "sympatique"],
	["antipatico, a, i, e", "antipatique"],
	["allegro, a, i, e", "joyeux"],
	["felice, i", "heureux"],
	["contento, a, i, e", "content"],
	["sorridente, i", "souriant"],
	["triste, i", "triste"],
	["cortese, i", "poli"],
	["scortese, i", "impoli"],
	["fortunato, a, i, e", "chanceux"],
	["sfortunato, a, i, e", "malchanceux"],
	["vero, a, i, e", "vrai"],
	["falso, a, i, e", "faux"],
	["interessante, i", "intéressant"],
	["I colori", "Les couleurs"],
	["il colore (masc.)", "la couleur"],
	["A colori.", "En couleurs."],
	["bianco, a, i, e", "blanc"],
	["nero, a, i, e", "noir"],
	["In bianco e nero.", "En noir et blanc."],
	["biondo, a, i, e", "blond"],
	["castano, a, i, e", "châtain"],
	["bruno, a, i, e", "brun"],
	["blu (inv.)", "bleu"],
	["viola (inv.)", "violet"],
	["rosa (inv.)", "rose"],
	["azzurro, a, i, e", "bleu ciel"],
	["celeste, i", "bleu clair"],
	["rosso, a, i, e", "rouge, roux"],
	["giallo, a, i, e", "jaune"],
	["verde, i", "vert"],
	["grigio, a, i, e", "gris"],
	["marrone, i", "brun, marron"],
	["arancione, i", "orange"],
	["arrivare*", "arriver"],
	["usare", "utiliser"],
	["conoscere", "connaître"],
	["corrispondere (corrisposto)", "correspondre"],
	["descrivere (descritto)", "décrire"],
	["prendere (preso)", "prendre"],
	["aprire (aperto)", "ouvrir"],
	["capire (-isco)", "comprendre"],
	["dare del tu (v. irregolare)", "tutoyer"],
	["dare del lei (v. irregolare)", "vouvoyer"],
	["ecco", "voici"],
	["ancora", "encore"],
	["sempre", "toujours"],
	["mai", "jamais"],
	["ma", "mais"],
	["dove", "où"],
	["Dove sei?", "Où es-tu?"],
	["quando", "quand"],
	["che", "quel(s), quelle(s)"],
	["Che lavoro fai?", "Quel est ton métier ?"],
	["In che anno sei nato / a?", "En quelle année es-tu né(e) ?"],
	["Sono nato / a nel 1998.", "Je suis né(e) en 1998."],
	["quale, i", "quel(s) ; quelle(s)"],
	["Quali lingue parli?", "Quelles langues parles-tu ?"],
	["alcuni, e", "quelques (+ noms au pluriel)"],
	["Alcuni uomini.", "Quelques hommes."],
	["ogni + singolare", "chaque"],
	["perché", "pourquoi; parce que"],
	["Buongiorno.", "Bonjour."],
	["Buonasera.", "Bonsoir."],
	["Buonanotte.", "Bonne nuit."],
	["Arrivederci.", "Au revoir."],
	["Come stai?", "Comment vas-tu ?"],
	["Bene, e tu?", "Bien, et toi"],
	["Di dove sei?", "D'où es-tu?"],
	["Dove abiti?", "Où habites-tu ?"],
	["Che fortuna!", "Quelle chance!"],
	["Complimenti!", "Félicitations!"],
	["Secondo me...", "Selon moi..."],
	["(Che) cosa significa... ?", "Que signifie... ?"],
	["Come si dice... in italiano?", "Comment dit-on... en italien ?"],
	["Come si scrive... ?", "Comment écrit-on... ?"],
	["Ho capito / Non ho capito.", "J'ai compris / Je n'ai pas compris."],
	["Può ripetere per favore?", "Pouvez-vous répéter s'il vous plaît ? (politesse)"],
	["È giusto? / È sbagliato?", "C'est juste ? / C'est faux ?"],
	["La forma di cortesia.", "La forme de politesse."],

]
const unit2 =  [
	["il tempo libero","les loisirs"],
	["il passatempo","le passe-temps, le loisir"],
	["la gita","l’excursion"],
	["il biglietto","le billet"],
	["lo spettacolo","le spectacle"],
	["l’arte (femm.)","l’art"],
	["la natura","la nature"],
	["la mostra (d’arte)","l’exposition (d’art)"],
	["il cinema","le cinéma"],
	["il teatro","le théâtre"],
	["la piscina","la piscine"],
	["la discoteca","la discothèque"],
	["il museo","le musée"],
	["la danza","la danse"],
	["la musica","la musique"],
	["il concerto","le concert"],
	["l’allenamento","l’entraînement"],
	["la gara","la course"],
	["la partita","le match"],
	["la palestra","la salle de gym"],
	["l’impegno","l’engagement"],
	["il videogioco","le jeu vidéo"],
	["il tango","le tango"],
	["il violino","le violon"],
	["lo strumento","l’instrument"],
	["l’amico","l’ami"],
	["le carte","le jeux de cartes"],
	["il gruppo","le groupe"],
	["l’intervista","l’interview"],
	["il giornalista","le journaliste"],
	["l’aeroporto","l’aéroport"],
	["il mare (masc.)","la mer"],
	["il lago","le lac"],
	["la montagna","la montagne"],
	["la vacanza","les vacances; le séjour"],
	["la casa","la maison"],
	["il palazzo","l’immeuble, le palais"],
	["il piano","l’étage"],
	["l’appartamento","l’appartement"],
	["il locale","l’établissement public"],
	["la stanza","la pièce"],
	["il soggiorno","le séjour"],
	["il salotto","le salon"],
	["la sala da pranzo","la salle à manger"],
	["la cucina","la cuisine"],
	["la camera","la chambre"],
	["la camera da letto","la chambre à coucher"],
	["il bagno","la salle de bain"],
	["il corridoio","le corridor"],
	["l’ingresso","l’entrée"],
	["il ripostiglio","le cagibi"],
	["l’affitto","le loyer"],
	["l’ascensore","l’ascenseur"],
	["il balcone","le balcon"],
	["il centro","le centre"],
	["la periferia","la banlieue"],
	["L’ora","L’heure"],
	["la mezzora","la demie-heure"],
	["l’orario","l’horaire"],
	["l’orario di apertura","l’horaire d’ouverture"],
	["l’orario di chiusura","l’horaire de fermeture"],
	["l’orologio (masc.)","la montre"],
	["Che ora è?","Quelle heure est-il?"],
	["Che ore sono?","Quelle heure est-il?"],
	["Sono le sette.","Il est sept heures."],
	["Sono le sei e un quarto.","Il est six heures et quart."],
	["Sono le otto meno dieci.","Il est huit heures moins dix."],
	["Sono le undici e mezzo(a).","Il est onze heures et demi."],
	["È l’una.","Il est une heure."],
	["È mezzogiorno","Il est midi."],
	["È mezzanotte.","Il est minuit."],
	["A che ora?","À quelle heure?"],
	["Alle diciotto.","À dix-huit heures."],
	["A mezzanotte.","À minuit."],
	["All’una.","À une heure."],
	["Fra mezz’ora.","Dans une demi-heure."],
	["Dalle 15 alle 17.","De 15 à 17 heures."],
	["Verso le 3.","Vers 3 heures."],
	["Fino alle 21.","Jusqu’à 21 heures."],
	["i giorni della settimana","les jours de la semaine"],
	["il giorno","le jour"],
	["la settimana","la semaine"],
	["il fine settimana","le week-end"],
	["la mattina (femm.)","le matin"],
	["Stamattina.","Ce matin."],
	["la sera (femm.)","le soir"],
	["Stasera.","Ce soir."],
	["il pomeriggio","l’après-midi"],
	["il lunedì","le lundi"],
	["il martedì","le mardi"],
	["il mercoledì","le mercredi"],
	["il giovedì","le jeudi"],
	["il venerdì","le vendredi"],
	["il sabato","le samedi"],
	["la domenica (femm.)","le dimanche"],
	["i mezzi di trasporto urbani","les moyens de transport urbains"],
	["il mezzo","le moyen"],
	["il trasporto","le transport"],
	["il tram","le tram"],
	["l’autobus","le bus"],
	["il treno","le train"],
	["l’aereo","l’avion"],
	["la fermata","l’arrêt"],
	["la stazione","la gare"],
	["la metropolitana (femm.)","le métro"],
	["il metrò","le métro"],
	["la linea","la ligne"],
	["il traffico","le trafic"],
	["il parcheggio","le parking"],
	["il motorino","le vélomoteur"],
	["la bicicletta","la biciclette"],
	["il taxi","le taxi"],
	["il tassista","le chauffeur de taxi"],
	["la gente (sing.)","les gens"],
	["moderno, a, i, e","moderne"],
	["antico, a, i, e","ancien"],
	["comodo, a, i, e","confortable"],
	["scomodo, a, i, e","inconfortable"],
	["luminoso, a, i, e","lumineux"],
	["scuro, a, i, e","sombre"],
	["nuovo, a, i, e","neuf"],
	["vecchio, a, i, e","vieux"],
	["abitare","habiter"],
	["accettare","accepter"],
	["chiacchierare","bavarder"],
	["passeggiare","se promener"],
	["giocare","jouer"],
	["suonare","jouer d’un instrument"],
	["ballare","danser"],
	["mangiare","manger"],
	["navigare","naviguer"],
	["studiare","étudier"],
	["visitare","visiter"],
	["invitare","inviter"],
	["organizzare","organiser"],
	["incontrare","rencontrer"],
	["informarsi*","s’informer"],
	["vedere (visto)","voir"],
	["andare* (v. irregolare) ","aller"],
	["stare* (v. irregolare)","rester; aller (bien ou mal)"],
	["dare (v. irregolare)","donner"],
	["fare (v. irregolare)","faire"],
	["potere (v. irregolare)","pouvoir"],
	["volere (v. irregolare)","vouloir"],
	["sapere (v. irregolare)","savoir"],
	["dovere (v. irregolare)","devoir"],
	["avere voglia di (v. irregolare)","avoir envie de"],
	["uscire* (v. irregolare)","sortir"],
	["venire* (v. irregolare)","venir"],
	["sempre","toujours"],
	["mai","jamais"],
	["di solito","d’habitude"],
	["regolarmente","régulièrement"],
	["raramente","rarement"],
	["spesso","souvent"],
	["qualche volta","quelques fois"],
	["a volte ","quelques fois"],
	["una volta a settimana","une fois par semaine"],
	["due volte al mese","deux fois par mois."],
	["ogni volta","chaque fois"],
	["ieri","hier"],
	["oggi","aujourd’hui"],
	["domani","demain"],
	["presto","tôt"],
	["tardi","tard"],
	["prima","avant"],
	["dopo","après"],
	["prossimo, a, i, e","prochain"],
	["La settimana prossima.","La semaine prochaine."],
	["scorso, a, i, e","passé"],
	["Il mese scorso.","Le mois passé."],
	["Che ne dici di (+inf.)…?","Qu’en dis-tu de…?"],
	["È un’ottima idea!","C’est une excellente idée!"],
	["Perché no?","Pourquoi pas?"],
	["Volentieri!","Volontiers!"],
	["Mi dispiace, ma non posso.","Désolé, mais je ne peux pas."],
	["Purtroppo ho già un impegno.","Malheureusement j’ai déjà un engagement."],
	["Mi piace il cinema.","J’aime le cinéma."],
	["Mi piacciono le gite.","J’aime les excursions."],
	["Andare* al cinema / a teatro.","Aller au cinéma / au théâtre."],
	["Andare* al mare / al lago.","Aller à la mer / au lac."],
	["Andare* in città / in centro.","Aller en ville / au centre."],
	["Andare* in piscina / in montagna.","Aller à la piscine / à la montagne."],
	["il primo, a, i, e","le premier"],
	["il secondo, a, i, e","le deuxième"],
	["il terzo, a, i, e","le troisième"],
	["il quarto, a, i, e","le quatrième"],
	["il quinto, a, i, e","le cinquième"],
	["il sesto, a, i, e","le sixième"],
	["il settimo, a, i, e","le septième"],
	["l’ottavo, a, i, e","le huitième"],
	["il nono, a, i, e","le neuvième"],
	["il decimo, a, i, e","le dixième"],
	["l’undicesimo","le onzième"],
	["il dodicesimo","le douzième"],
	["il ventitreesimo","le vingt-troisième"],
	["il venticinquesimo","le vingt-cinquième"],
	["il ventiseiesimo","le vingt-sixième"],
	["I numeri","Les nombres"],
	["venti","vingt"],
	["trenta","trente "],
	["quaranta","quarante"],
	["cinquanta","cinquante"],
	["sessanta","soixante"],
	["settanta","septante"],
	["ottanta","huitante"],
	["novanta","nonante"],
	["cento","cent"],
	["duecento","deux-cents"],
	["trecento","trois-cents"],
	["mille","mille"],
	["milleduecento","mille deux-cents"],
	["duemila","deux mille"]
]

export const vocabulare = [
	unit0,
	unit1,
	unit2
]
