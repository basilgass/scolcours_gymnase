		...
		<div class="flex flex-col gap-3 border-b border-gray-200 px-5 py-3 mb-5">
			<div class="flex w-full justify-between items-baseline">
				...
				<div class="flex gap-3 justify-end">
					...
					<Link class="btn-cancel btn-xs" :href="route('questions.show', [theQuestion.id])">
					retour
					</Link>
					<confirm-button class="btn-delete btn-xs" @confirm="deleteQuestion">
						supprimer
					</confirm-button>
				</div>
			</div>

			<move-item-to :source-id="theQuestion.id" source="question" target="post" />
		</div>
		<div class="flex flex-col md:flex-row gap-3 px-5 pb-5">
			<form class="flex-1 flex flex-col gap-2 font-code" @submit.prevent>
				<form-maker v-model="theQuestion.block.title" inline-label label="titre" label-class="w-[60px]" sm
					type="text" />

				<form-maker v-model="theQuestion.css" inline-label label="CSS" label-class="w-[60px]" sm type="text" />

				<form-maker ref="formBody" v-model="theQuestion.block.body"
					:label="`body (id: ${theQuestion.block.id})`" :rows="10" language="latex"
					message="\$a = TeX, \$A = texte, @$A = format spéciaux" message-class="text-[12px]" type="code" />

				<form-maker v-model="theQuestion.answer" :rows="3" label="answer" message="multi-réponse: ||"
					message-class="text-[12px]" name="answer" type="textarea" />

				<form-maker v-model="theQuestion.keyboard" :rows="8" label="clavier" type="keyboard" />
			</form>

			<question-show :class="theQuestion.css" :question="theQuestion" class="min-w-[350px] md:max-w-[40vw]"
				is-dynamic show-input show-title />
		</div>
	</section>
</template>
