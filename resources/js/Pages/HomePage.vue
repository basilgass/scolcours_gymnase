...
<template>
	<div class="space-y-10 bg-gray-100">
		...
		<div class="p-5 bg-red-100 border border-red-600 shadow text-red-700">
			Attention - le site web est en cours de développement. Certaines fonctionnalités peuvent ne pas être
			disponibles.
		</div>

		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
			<Link v-for="(theme, index) in $page.props.themes" :key="theme.slug" :class="`bg-scolcours-${theme.slug} ${index % 2 === 0 ? 'rotate-1' : '-rotate-1'
				}`" :href="'/' + theme.slug"
		...
		<div class="space-y-10">
			...
			<div class="grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
				<div v-for="item of newChapters" :key="item.id"
					...
					<div class="flex justify-between items-center mb-2">
						<h3 class="font-normal text-lg" v-katex.auto="item.title" />
