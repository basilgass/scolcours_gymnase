...
const cesarCoded = computed(() => Cipher.affine(cipherKey.value, clearText.value))

const showTable = computed(() => !parameters.value['!table'])
const cipherAlphabet = computed(() => Cipher._affineAlphabet(cipherKey.value))
						...
						<div class="flex row">
							<div
								...
								:class="[
									...
									'cell grid place-items-center',
									`letter-${alpha}`,
									(highlight===index ) ? 'bg-red-300 font-semibold' : ''
								]"
								@mouseenter="highlight=index"
								@mouseleave="highlight=-1"
							>
