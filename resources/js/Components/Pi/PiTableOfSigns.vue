...
const tos = computed(() => {
	...
	// BUG : PiMath.tableOfSigns utilise la recherche des zéros - si trop complexe, on se retrouve avec une boucle trop longue et un freeze !
...
function createPolyFactor() {
	// TODO: split at / outside of parenthesis ? Allow complex poly factors
	const [num, den] = props.fx.split("/")

	if (den === undefined) {
		return new PolyFactor().fromPolynom(props.fx).factorize().reduce()
	}

	return new PolyFactor().fromPolynom(num, den).factorize().reduce()
}
