		...
		<div class="keyboard-output">
			...
			>
				<div
					...
					class="self-center font-code"
					v-text="props.modelValue"
				/>
			</div>
		</div>

		<!-- new version of the keyboard -->
		<div class="flex flex-col w-full">
			<!-- numeric keyboard -->
			<div class="flex flex-col gap-2 md:flex-row">
				<div class="grid grid-cols-3 gap-2 keyboard">
					<button
						v-for="(key, index) of numericKeyboard"
				...
				<div class="grid grid-cols-3 md:grid-cols-2 gap-2 keyboard">
					<button
						v-for="(key, index) of extendedKeyboard"
						:key="`key-${key.key}-${index}`"
						class="key"
						:class="`${keyClass} ${key.span===0?'':key.span} ${key.visible?'invisible':''}`"
						@click="ButtonKeyClick(key)"
...
function generateKeyboard(kbrd) {
	let data = []

	// Loop through all keyboard keys in the layout.
	for (let key of kbrd.layout) {
		...
		if (kbrd.keys !== undefined && kbrd.keys[kkey] !== undefined) {
			kdata.type = kbrd.keys[kkey].type === undefined ? kdata.type : kbrd.keys[kkey].type
			kdata.display = kbrd.keys[kkey].display === undefined ? kdata.display : kbrd.keys[kkey].display
			kdata.fn = kbrd.keys[kkey].fn === undefined ? kdata.fn : kbrd.keys[kkey].fn
		}
...
let numericKeyboard = computed(() => {
	return generateKeyboard(keyboards["number"])
})
let extendedKeyboard = computed(()=>{
	let data = generateKeyboard(keyboardData.value)
	console.log(data)
})
let keyboardComputed = computed(() => {
		return generateKeyboard(keyboardData.value)
	}),
