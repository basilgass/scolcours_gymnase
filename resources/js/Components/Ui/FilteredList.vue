				...
				<transition-group name="list">
					...
					>
						...
						<div
							...
							class="p-4 border border-gray-200 rounded hover:scale-105 hover:shadow transition-all"
							:class="props.itemClass"
						>
							<Link
								v-if="props.routeName!==''"
								v-katex.auto="props.itemTitle(item)"
								:href="route(props.routeName, props.routeData(item))"
							/>
							<div
								v-else
								v-katex.auto="props.itemTitle(item)"
...
const props = defineProps({
	...
	routeData: {type: Function, default: (item)=>[]},
	itemTitle: {type: Function, default: (item)=>item?.title},
	itemBackground: {type: Function, default: (item)=>"bg-white"},
