...
let keyboardComputed = computed(() => {
	...
	keyboardCommands = computed(() => {
		const btnReset = { label: 'tout effacer', icon: 'bi bi-trash', span: 1, fn: () => resetKeyStrokes(), atEnd: false },
		...
		if (props.back) {smallButtons++}
		if (props.reset) {smallButtons++}
		if (props.next) {smallButtons++}
