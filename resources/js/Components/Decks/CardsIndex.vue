...
function addCard() {
	axios.post(route("decks.addCard", [props.deck.id]))
		.then(res => {
			theDeck.value.cards.push(res.data)
		})
		.catch(err => {
...
function deleteCard(cardId) {
	...
	// 	.then(() => {
	// 		theDeck.value.cards = theDeck.value.cards.filter(card => card.id !== cardId)
	// 	})
}
...
function updateDecksOrder() {
	axios
		.post(route("decks.updateOrder"), {
			order: theDeck.value.cards.map((x, index) => {
				// return { id: x.id, order: index }
				// BUG: index is not the same as the id
				return { id: index, order: index }
			})
		})
		.then((res) => {
			console.log(res)
