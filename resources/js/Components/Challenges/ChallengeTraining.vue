<script setup>
	...
	let generateQuestion = function () {
		...
		theQuestion = computed(() => {
			if (question.value.question === undefined) {
				return false
			}

			return useGenerators().question(question, props.challenge)

			// return {
			// 	block: {
			// 		body: props.challenge.output
			// 			.replace("question", question.value.question)
			// 			.replace("answer", "$a"),
			// 		illustrations: [],
			// 	},
			// 	keyboard: question.value.keyboard
			// 		? question.value.keyboard
			// 		: props.challenge.keyboard,
