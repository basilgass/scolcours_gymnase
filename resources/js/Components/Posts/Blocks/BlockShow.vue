...
const blockTypes = useBlockTypes,
	...
	blockTemplate = computed(()=>{
		...
		const values = theBlock.value.template.split(",").filter(x=>x!=="")
		// pattern is:
		...
		let grid = ["grid grid-cols-1 gap-3"],
			block = [values[0][0]==="b"?"order-1 col-span-1":"order-2 col-span-1"],
			illustration = [values[0][0]==="i"?"order-1 col-span-1":"order-2 col-span-1"]
			...
			if(["md","lg", "xl"].indexOf(media[0])!==-1){
				if(media[1].includes("+")){
					...
					const value = media[1].split("+"),	// ["2b", "i"]
						sizes = [
							value[0].slice(0,-1)===""?1:+value[0].slice(0, -1),
							value[1].slice(0,-1)===""?1:+value[1].slice(0, -1)
						] // [2, 1]
					...
					for(let j=0; j<sizes.length; j++){
						if(sizes[j]===0){
							...
							if(value[j].includes("b")){
								block.push(`${media[0]}:order-${j+1} ${media[0]}:col-span-${sizes[j]}`)
							}else{
								illustration.push(`${media[0]}:order-${j+1} ${media[0]}:col-span-${sizes[j]}`)
							}
						}
					}
					...
					block.push(media[1][0]==="b"?`${media[0]}:order-1 ${media[0]}:col-span-1`:`${media[0]}:order-2 ${media[0]}:col-span-1`)
					illustration.push(media[1][0]==="i"?`${media[0]}:order-1 ${media[0]}:col-span-1`:`${media[0]}:order-2 ${media[0]}:col-span-1`)
				}
			}
		}
